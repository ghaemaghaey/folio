...
function restoreEpubFromCharOffset(chapter, co) {
  const content = el.epubContent;
  if (!content) return;
  ...
  while ((node = walk2.nextNode())) {
    ...
    if (charCount + text.length >= targetChar) {
      ...
      while (targetNode && targetNode !== section && !["p","div","h1","h2","h3","h4","h5","h6","li","blockquote","pre","section","article"].includes(targetNode.tagName?.toLowerCase())) {
...
function queueSync(fingerprint, position, device) {
  ...
  // Deduplicate: keep only the newest entry per fingerprint+device
...
/**
 * Push current reading position to folio-server.
 * On network failure, queues the sync for later retry.
 */
...
async function loadCatalogInitial() {
  ...
  try {
    ...
    // Cache successful catalog for offline use
    ...
    if (seq !== state.catalog.searchSeq) return;
    console.error(err);
    // Fall back to cached catalog if network fails
...
async function boot() {
  ...
  requestAnimationFrame(() => {
    ...
    setTimeout(async () => {
      ...
      // Drain pending syncs/uploads when coming online
      if (navigator.onLine) {
        drainSyncQueue().catch(() => {});
        drainUploadQueue().catch(() => {});
      }
    }, 0);
  });
}
